...
export const IslandRepository = {
    // async function
    async getIslandById(id) {
        // await response of fetch call, only return once promise is resolved
        return await fetchIt(`${Settings.remoteURL}/islands/${id}`)
    },
    async createIsland(island) {
        return await fetchIt(`${Settings.remoteURL}/islands`, "POST", JSON.stringify(island))
    },
    async delete(id) {
        return await fetchIt(`${Settings.remoteURL}/islands/${id}`, "DELETE")
    },
    async getAllIslands() {
        return await fetchIt(`${Settings.remoteURL}/islands`)
    }
}
